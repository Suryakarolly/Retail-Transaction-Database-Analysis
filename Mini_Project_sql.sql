E-COMMERCE RETAIL DATA ANALYSIS -SQL PROJECT1
==============================================
...
SET SQL_SAFE_UPDATES = 0;    #SAFE UPDATES is a safeguard in MySQL that prevents accidental updates or deletions in your database
...
SELECT 
    ...
    TIMESTAMPDIFF(DAY, MIN(`tran_date`), MAX(`tran_date`)) AS total_days,       #The TIMESTAMPDIFF function in MySQL is used to calculate the difference between two date or datetime expressions.
...
---return mean the prodcut was returned by the customers, so we need to take ( -ve ) value as a return value
---if quty have -6 and total_amount is -158
---so return qty is 6 and return amount is 158 
...
SELECT Store_type,total_quantity_sold,total_sales_amount
FROM 
     SalesByStoreType
WHERE 
...
SELECT p.prod_cat,c.avg_revenue
FROM CategoryAvgRevenue c
JOIN OverallAvgRevenue o ON c.avg_revenue > o.overall_avg_revenue
JOIN prod_cat_info p ON c.prod_cat_code = p.prod_cat_code;
